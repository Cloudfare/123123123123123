...
void CAimbot::Run()
{
	...
	if (target_index != -1 && current_aim_position != Vector(0, 0, 0) && pTarget)
	{
		...
		if (RagebotElements.Ragebot_AutoDuck->Checked)
		{
			/*if (RagebotElements.Ragebot_AutoDuck_Perfect->Checked)
			{
				float damage = 0.f;
				if (EngineTrace->IsVisible(pLocal, vLocalEyes, vTargetDest, pBaseEntity) || Autowall::CanHit(vLocalEyes, vTargetDest, damage))
				{
...
{
	if (RagebotElements.Ragebot_AutoRevolver->Checked && G::MainWeapon->WeaponID() == ItemDefinitionIndex::REVOLVER)
	{
		G::UserCmd->buttons |= IN_ATTACK;
