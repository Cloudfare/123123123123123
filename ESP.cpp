...
void CESP::DrawPlayer(CBaseEntity* pPlayer, CBaseEntity* pLocalPlayer)
{
	...
	if (GameUtils::WorldToScreen(m_position, footpos) && GameUtils::WorldToScreen(m_top_position, headpos))
	{
		...
		if (!isEnemyVis && VisualElements.Visual_Player_VisibleOnly->Checked)
			return;
		...
		if (VisualElements.Visual_Player_Trace->Checked && pPlayer != pLocalPlayer)
		{
			auto Draw3DBox = [](Vector Org, Vector vMin, Vector vMax, D3DCOLOR col)
			{
				Vector _min = Org + vMin;
				Vector _max = Org + vMax;
				Vector min, max;
				...
				if (GameUtils::WorldToScreen(_min, min) &&
					...
					GameUtils::WorldToScreen(crnr7, crnr7_))
				{
					g_pRender->Line(min.x, min.y, crnr2_.x, crnr2_.y, col);
					g_pRender->Line(min.x, min.y, crnr4_.x, crnr4_.y, col);
					g_pRender->Line(min.x, min.y, crnr6_.x, crnr6_.y, col);
					g_pRender->Line(max.x, max.y, crnr5_.x, crnr5_.y, col);
					g_pRender->Line(max.x, max.y, crnr7_.x, crnr7_.y, col);
					g_pRender->Line(max.x, max.y, crnr3_.x, crnr3_.y, col);
					g_pRender->Line(crnr2_.x, crnr2_.y, crnr7_.x, crnr7_.y, col);
					g_pRender->Line(crnr2_.x, crnr2_.y, crnr3_.x, crnr3_.y, col);
					g_pRender->Line(crnr4_.x, crnr4_.y, crnr5_.x, crnr5_.y, col);
					g_pRender->Line(crnr4_.x, crnr4_.y, crnr3_.x, crnr3_.y, col);
					g_pRender->Line(crnr6_.x, crnr6_.y, crnr5_.x, crnr5_.y, col);
					g_pRender->Line(crnr6_.x, crnr6_.y, crnr7_.x, crnr7_.y, col);
				}
			};

			Vector traceEndPos;
			Vector vecForward, vecStart = pPlayer->GetEyePosition();

			Math::AngleVectors(pPlayer->GetEyeAngles(), &vecForward);

			if (pMainWeapon)
			{

				auto sWeaponInfo = pMainWeapon->GetCSWpnData();
				...
				//DebugOverlay->AddLineOverlay(tr.startpos, traceEndPos, r, g, b, true, Globals->interval_per_tick);
				Vector screenEnd, screenStart;
				//if (GameUtils::WorldToScreen(tr.startpos, screenStart) && GameUtils::WorldToScreen(traceEndPos, screenEnd))
				//{
				GameUtils::WorldToScreen(tr.startpos, screenStart);
				GameUtils::WorldToScreen(traceEndPos, screenEnd);
				//so that not both have to be visible, gg ez fix
					g_pRender->Line(screenStart.x, screenStart.y, screenEnd.x, screenEnd.y, WHITE(255));
					Draw3DBox(traceEndPos, Vector(-2, -2, -2), Vector(2, 2, 2), col);
				//}
				//DebugOverlay->AddBoxOverlay(traceEndPos - Vector(0, 0, 0), Vector(-2, -2, -2), Vector(2, 2, 2), QAngle(pBaseEntity->GetEyeAngles().x, pBaseEntity->GetEyeAngles().y, 0), r, g, b, 50, Globals->interval_per_tick);
		...
		if (VisualElements.Visual_Player_Health->Checked)
		{
			...
			if (pPlayer->GetHealth() < 100)
			{
				...
				g_Draw.StringA(g_Draw.font_esp, false, x - w - g_Draw.getWidht(hp, g_Draw.font_esp), y + health*height / 100 - 12, 255, 255, 255, 255, "%i", pPlayer->GetHealth());
			}
			int step_height = height / 4;
			for (int i = 1; i < 4; i++)
			{
		...
		if (VisualElements.Visual_Player_Name->Checked)
		{
			...
			g_Draw.StringA(g_Draw.font_esp, true, x + width / 2, y - 12 - top_add, 255, 255, 255, 255, "%s", info.m_szPlayerName);

			top_add += 12;

			if(pPlayer->GetFlags() & FL_ONGROUND)
				g_Draw.StringA(g_Draw.font_esp, true, x + width / 2, y - 12 - top_add, 255, 255, 255, 255, "AA: %s", g_Resolver->aa_info[pPlayer->Index()].c_str());
		//	else
//		//	g_Draw.StringA(g_Draw.font_esp, true, x + width / 2, y - 12 - top_add, 255, 255, 255, 255, "Resolver Stage: %i", g_Resolver->resolve_infos[pPlayer->Index()].stage);


			

		}
		if (VisualElements.Visual_Player_Weapon->SelectedIndex > 0)
		{
			...
			if (hWeapons)
			{
				...
				if (pMainWeapon)
				{
					...
					if(VisualElements.Visual_Player_Weapon->SelectedIndex == 1)
						g_Draw.StringA(g_Draw.font_esp, true, x + width / 2, y + height + bot_add, 124, 252, 0, 255, XorStr("%s"), s1.c_str());
					else
					{
						...
						g_Draw.StringA(g_Draw.font_weapon, false, x + width / 2 - (icon_width + ammo_width)/2, y + height + bot_add, 255, 255, 255, 255, XorStr("%s"), pMainWeapon->GetGunIcon());
						g_Draw.StringA(g_Draw.font_esp, false, x + width / 2 - (icon_width + ammo_width) / 2 + icon_width, y + height + bot_add, 255, 255, 255, 255, XorStr("%s"), ammo.c_str());
					}
					...
					if (VisualElements.Visual_Player_Weapon_All->Checked)
					{
						for (int nIndex = 0; hWeapons[nIndex]; nIndex++) //so we wanna start with 1 and do not use the primary
						{
							...
							g_Draw.StringA(g_Draw.font_esp, true, x + width / 2, y + height + bot_add, 255, 255, 255, 255, XorStr("%s"), s.c_str());
...
void CESP::DrawHostage(CBaseEntity* pHostage)
{
	...
	if (VisualElements.Visual_CBaseEntity_Name->Checked)
	{
		...
		g_Draw.StringA(g_Draw.font_esp, true, Left + width / 2 , Bot.y, 255, 255, 255, 255, "Hostage");
...
void CESP::DrawDroppedWeapon(CBaseCombatWeapon* pWeapon)
{
	...
	if (!sCurWeapon.empty() && sCurWeapon.find(XorStr("w_")) != std::string::npos)
	{
		...
		if (VisualElements.Visual_CBaseEntity_Name->Checked)
		{
			g_Draw.StringA(g_Draw.font_esp, true, Left + width / 2, Bot.y, 255, 255, 255, 255,  name.c_str());
		}	
	}
}
...
void CESP::Draw()
{
	...
	//	bool is_visible = draw_array.at(i).is_visible;
		...
		if (VisualElements.Visual_Player_Trace->Checked && pPlayer != G::LocalPlayer)
		{
			auto Draw3DBox = [](Vector Org, Vector vMin, Vector vMax, D3DCOLOR col)
			{
				Vector _min = Org + vMin;
			...
			if (pMainWeapon)
			{
				...
				//so that not both have to be visible, gg ez fix
				g_pRender->Line(screenStart.x, screenStart.y, screenEnd.x, screenEnd.y, WHITE(255));
				Draw3DBox(traceEndPos, Vector(-2, -2, -2), Vector(2, 2, 2), color);
				//}
				//DebugOverlay->AddBoxOverlay(traceEndPos - Vector(0, 0, 0), Vector(-2, -2, -2), Vector(2, 2, 2), QAngle(pBaseEntity->GetEyeAngles().x, pBaseEntity->GetEyeAngles().y, 0), r, g, b, 50, Globals->interval_per_tick);
		...
		if (VisualElements.Visual_Player_Name->Checked)
		{
			...
			/*for (int i = 0; i < g_BacktrackHelper->PlayerRecord[pPlayer->Index()].records.size(); i++)
			{
				g_pRender->String(x + width / 2, y - 12 - top_add, centered, g_pRender->Fonts.esp, true, WHITE(255), "Record: %i - %f", i, g_BacktrackHelper->PlayerRecord[pPlayer->Index()].records.at(i).m_flSimulationTime);
				top_add += 12;
			}*/
			g_pRender->String(x + width / 2, y - 12 - top_add, centered, g_pRender->Fonts.esp, true, WHITE(255), "AA: %s", g_Resolver->aa_info[pPlayer->Index()].c_str());
		}
		if (VisualElements.Visual_Player_Weapon->SelectedIndex >= 0)
		{
			UINT* hWeapons = pPlayer->GetWeapons();
...
void CESP::Loop()
{
	...
	//HitChance(NULL, 8092, 50);
	
	if (G::LocalPlayer && G::LocalPlayer->GetHealth() > 0 && G::MainWeapon)
	{
		//HitChance(NULL, 8018.f, 100);
