...
int RebuildGameMovement::TryPlayerMove(CBaseEntity *player, Vector *pFirstDest, trace_t *pFirstTrace)
{
	...
	time_left[player->GetIndex()] = g_pGlobals->frametime;   // Total time for this movement operation.

	new_velocity[player->GetIndex()].Init();
	...
	for (bumpcount[player->GetIndex()] = 0; bumpcount[player->GetIndex()] < numbumps[player->GetIndex()]; bumpcount[player->GetIndex()]++)
	{
			...
			player->GetFlags() & FL_ONGROUND)
		{
			for (i[player->GetIndex()] = 0; i[player->GetIndex()] < numplanes[player->GetIndex()]; i[player->GetIndex()]++)
			{
				...
				{
					ClipVelocity(original_velocity[player->GetIndex()], planes[i[player->GetIndex()]], new_velocity[player->GetIndex()], 1.0 + g_pCvar->FindVar("sv_bounce")->GetValue() * (1 - player->GetFriction()));
				}
			}
...
void RebuildGameMovement::Accelerate(CBaseEntity *player, Vector &wishdir, float wishspeed, float accel)
{
	...
	accelspeed[player->GetIndex()] = accel * g_pGlobals->frametime * wishspeed * player->GetFriction();
...
void RebuildGameMovement::AirAccelerate(CBaseEntity *player, Vector &wishdir, float wishspeed, float accel)
{
	...
	accelspeed[player->GetIndex()] = accel * wishspeed * g_pGlobals->frametime * player->GetFriction();
	...
	{
		player->GetVelocity()[i[player->GetIndex()]] += accelspeed[player->GetIndex()] * wishdir[i[player->GetIndex()]];
		g_pMoveHelper->SetHost(reinterpret_cast<CBaseEntity*>(player));
...
void RebuildGameMovement::AirMove(CBaseEntity *player)
{
	...
	AirAccelerate(player, wishdir[player->GetIndex()], wishspeed[player->GetIndex()], g_pCvar->FindVar("sv_airaccelerate")->GetValue());
...
void RebuildGameMovement::TracePlayerBBox(const Vector &start, const Vector &end, unsigned int fMask, int collisionGroup, trace_t& pm, CBaseEntity *player)
{
	...
	ray.Init(start, end, player->GetCollision()->VecMins(), player->GetCollision()->VecMaxs());
	g_pEngineTrace->TraceRay(ray, fMask, &filter, &pm);
}

void RebuildGameMovement::WalkMove(CBaseEntity *player)
{
	Math::AngleVectors(player->GetEyeAngles(), forward[player->GetIndex()],right[player->GetIndex()], up[player->GetIndex()]);  // Determine movement angles
	...
	Accelerate(player, wishdir[player->GetIndex()], wishspeed[player->GetIndex()], g_pCvar->FindVar("sv_accelerate")->GetValue());
	player->GetVelocity()[2] = 0;
	...
	dest[player->GetIndex()][0] = player->GetOrigin()[0] + player->GetVelocity()[0] * g_pGlobals->frametime;
	dest[player->GetIndex()][1] = player->GetOrigin()[1] + player->GetVelocity()[1] * g_pGlobals->frametime;
	...
	if (pm[player->GetIndex()].fraction == 1)
	{
		player->SetOrigin(pm[player->GetIndex()].endpos);
		// Now pull the base velocity back out.   Base velocity is set if you are on a moving object, like a conveyor (or maybe another monster?)
		VectorSubtract(player->GetVelocity(), player->GetVelocity(), player->GetVelocity());
...
void RebuildGameMovement::FinishGravity(CBaseEntity *player)
{
	...
	ent_gravity = g_pCvar->FindVar("sv_gravity")->GetValue();

	// Get the correct velocity for the end of the dt 
	player->GetVelocity()[2] -= (ent_gravity * g_pCvar->FindVar("sv_gravity")->GetValue() * g_pGlobals->frametime * 0.5);
...
void RebuildGameMovement::Friction(CBaseEntity *player)
{
	...
	{
		friction[player->GetIndex()] = g_pCvar->FindVar("sv_friction")->GetValue() * player->GetFriction();

		//  Bleed off some speed, but if we have less than the bleed
		//  threshold, bleed the threshold amount.


		control[player->GetIndex()] = (speed[player->GetIndex()] < g_pCvar->FindVar("sv_stopspeed")->GetValue()) ? g_pCvar->FindVar("sv_stopspeed")->GetValue() : speed[player->GetIndex()];

		// Add the amount to the drop amount.
		drop[player->GetIndex()] += control[player->GetIndex()] * friction[player->GetIndex()] * g_pGlobals->frametime;
	}
...
void RebuildGameMovement::CheckFalling(CBaseEntity *player)
{
	...
	if (!player->GetHealth() && player->GetFallVelocity() >= 303.0f)
	{
		...
		if (player->GetVelocity().z < 0.0f)
		{
			...
			player->SetFallVelocity(max(0.1f, player->GetFallVelocity() + player->GetVelocity().z));
		}
...
void RebuildGameMovement::CheckVelocity(CBaseEntity *player)
{
	...
	for (i[player->GetIndex()] = 0; i[player->GetIndex()] < 3; i[player->GetIndex()]++)
	{
		...
		// Bound it.
		if (player->GetVelocity()[i[player->GetIndex()]] > g_pCvar->FindVar("sv_maxvelocity")->GetValue())
		{
			player->GetVelocity()[i[player->GetIndex()]] = g_pCvar->FindVar("sv_maxvelocity")->GetValue();
		}
		else if (player->GetVelocity()[i[player->GetIndex()]] < -g_pCvar->FindVar("sv_maxvelocity")->GetValue())
		{
			player->GetVelocity()[i[player->GetIndex()]] = -g_pCvar->FindVar("sv_maxvelocity")->GetValue();
		}
	}
...
void RebuildGameMovement::StartGravity(CBaseEntity *player)
{
	...
	pVel[2] -= (g_pCvar->FindVar("sv_gravity")->GetValue() * 0.5f * g_pGlobals->interval_per_tick);
	pVel[2] += (player->GetVelocity()[2] * g_pGlobals->interval_per_tick);
